...
class rag_client:
    embedding_model = load_embedding_model(model_name="BAAI/bge-large-en-v1.5") #model_name="openai"
    ...
    def retrieve_context_reranked(self, query):
        return retrieve_context_reranked(
            query, retriever=self.retriever, reranker_model="colbert" # reranker_model="cohere" # colbert for local model
        )
